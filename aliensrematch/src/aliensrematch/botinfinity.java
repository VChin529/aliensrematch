...
public class botinfinity {
		...
		if (alienScan()) {
			...
			for (int i = 0; i < board.board.length; i++) {
				for (int j = 0; j < board.board.length; j++) {
					...
					// alien must be in scan range && open
					// if so, add up these values
			...
			for (cell[] currline : board.board) {
				for(cell curr: currline) {
					// if these coordinates are within the alien scanner range && open
					// multiply it by 1/total probability within scanner
					if (alienScanCoord(curr.x,curr.y) && curr.state) {
						curr.palien *= 1.0 / prob_square_total;
						// if these coordinates are outside the alien scanner range
						// multiply it by 1- (total probability outside of scanner)
					}else if (curr.state){
			...
			for (int i = 0; i < board.board.length; i++) {
				for (int j = 0; j < board.board.length; j++) {
					cell curr = board.board[i][j];
					// open cell that is not in alien scanner range
					// normalize upon these, so add to beta
					if (curr.state && !alienScanCoord(i, j)) {
						beta += curr.palien;
					} else if (curr.state) {
						curr.palien = 0.0;
					}
	...
	void alienMoveAlienProbability() {
		...
		for (int i = 0; i < board.board.length; i++) {
			for (int j = 0; j < board.board.length; j++) {
				cell curr = board.board[i][j];
				// if we are at our position x,y then probability of alien is 0
				if ((x==i && y==j)) {
					curr.palien = 0;
					// calculating all the ways an alien can move into the cell
				} else if (curr.state){
					curr.palien = 0;
					cell n = curr.up;
				...
				// add up total belief that an alien is in the detection range
				prob_square_total += curr.palien;
			}
		}
		...
		for (int i = 0; i < board.board.length; i++) {
			for (int j = 0; j < board.board.length; j++) {
				cell curr = board.board[i][j];
				// if it is within the scanner range
				// multiply by total belief in square
				if (curr.state && alienScanCoord(i,j)) {
					curr.palien*= (1.0/prob_square_total);
					// if it is outside scanner range
					// multiply by 1 - (1/prob_square_total)
				} else if (curr.state) {
					curr.palien *= (1.0/(1.0-prob_square_total));
				}
				beta += curr.palien;
